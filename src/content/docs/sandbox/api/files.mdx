...
```ts
await sandbox.gitCheckout(repoUrl: string, options?: GitCheckoutOptions): Promise<GitCheckoutResult>
```
...
- `options` (optional):
  ...
  - `depth` - Clone depth for shallow clones (positive integer). Use `1` to clone only the latest commit. Reduces bandwidth and speeds up cloning for large repositories.

**Returns**: `Promise<GitCheckoutResult>` containing:
- `success` - Whether the clone succeeded
- `repoUrl` - Repository URL that was cloned
- `branch` - Branch that was checked out
- `targetDir` - Directory where repository was cloned
- `timestamp` - Clone completion timestamp
...
await sandbox.gitCheckout('https://github.com/torvalds/linux', {
  depth: 1
});
...
await sandbox.gitCheckout('https://github.com/facebook/react', {
  branch: 'main',
  depth: 10,
  targetDir: '/workspace/react-src'
});
```
</TypeScriptExample>

:::note[Validation]
The `depth` parameter must be a positive integer. Values of 0, negative numbers, floats, or non-numeric values are rejected with a validation error.
:::
