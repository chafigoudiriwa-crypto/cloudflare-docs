...
- **Use shallow clones** - For large repositories, use `depth: 1` to clone only the latest commit. This significantly reduces bandwidth and clone time. Use higher depth values (like `depth: 10`) when you need limited commit history.
